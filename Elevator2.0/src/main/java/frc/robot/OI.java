...
public class OI {
  ...
  public OI() {
    ...
    //X.whenPressed(new SetElevator(4096));
    //Y.whenPressed(new SetElevator(2000));
	//B.whenPressed(new SetElevator(100)); 
	while(Elevator.getInstance().getElevatorPosition() < 24100){
		RB.whileHeld(new ElevatorWithJoystick(1));
	}
    while(Elevator.getInstance().getElevatorPosition() < 10){
		LB.whileHeld(new ElevatorWithJoystick(-.6));
	}
	//LB.whileHeld(new ElevatorWithJoystick(-.6));
	//RB.whileHeld(new ElevatorWithJoystick(1));
    X.whenPressed(new Hatch());
    Y.whileHeld(new Intake(1));
    B.whileHeld(new Intake(-1));
  }
}
